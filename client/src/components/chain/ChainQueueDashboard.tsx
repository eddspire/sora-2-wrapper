import { Clock, CheckCircle, AlertCircle, DollarSign, Film, PlayCircle, XCircle } from "lucide-react";
import type { ChainJob } from "@shared/schema";

interface ChainQueueDashboardProps {
  chains: ChainJob[];
}

export function ChainQueueDashboard({ chains }: ChainQueueDashboardProps) {
  const stats = {
    total: chains.length,
    queued: chains.filter(c => c.status === "queued").length,
    processing: chains.filter(c => c.status === "planning" || c.status === "generating" || c.status === "concatenating").length,
    completed: chains.filter(c => c.status === "completed").length,
    failed: chains.filter(c => c.status === "failed").length,
  };

  const totalCost = chains
    .filter(c => c.status === "completed" && c.costDetails)
    .reduce((sum, chain) => {
      try {
